...
const BlogDetails = () => {
    ...
    if(error){
        ...
        <div className="text-7xl font-bold text-red-500 font-tangerine grid place-content-center h-screen">
            {" "}
            ⚠️ An error occured</div>)
    }

    return (<div className="min-h-screen p-10 relative">
        <button 
        onClick={() => history("/blogs")}
        className="italic text-cyan-800 underline text-sm">Go back</button>
    {data?.blog&&(
        <div className="">
        <h1 className="font-bold uppercase text-2xl text-center">{data.blog.title}</h1>
        ...
        <div className="flex justify-between flex-col sm:flex-row items-center">
            ...
            <p><span className="font-bold font-mono text-cyan-700 underline">Updated:</span>{new Date(data.blog.updatedAt).toLocaleString()}</p>
        </div>
    </div>)}
