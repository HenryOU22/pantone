...
const Blogs = () => {
    ...
    return ( <>
    <h1 className="text-center font-tangerine text-rose-500 text-3xl sm:text-5xl my-5 font-bold">Blogs</h1>
    ...
    <div className="min-h-screen flex flex-col px-5 sm:px-10 gap-5">
        ...
        {error && <h1 className="font-bold animate-pulse text-red-800 m-auto text-5xl">{error}</h1>}

        {data && data.blogs.map((blog)=>(
            <div key={blog._id} className="group relative flex flex-col gap-2 rounded shadow-md p-5 mb-5 transition hover:shadow-xl">
                <h1 className="font-bold text-rose-500 text-2xl">{blog.title}</h1>
                <p className="text-gray-500">{blog.body.slice(onabort,200)}</p>
                <div className="w-full flex justify-between">
                    <p className="font-mono font-bold uppercase text-cyan-700">{blog.author}</p>
